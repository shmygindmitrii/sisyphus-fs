...
lowfat_fs* lowfat_fs_create_instance(uint32_t cluster_size, uint32_t cluster_count, uint32_t filename_length, uint8_t* mem) {
    ...
    fs_ptr->_data = mem;
    fs_ptr->_header = (lowfat_fs_header*)mem;
    ...
    LOWFAT_FS_ASSERT(fs_ptr->_info._system_used_clusters < cluster_count);
    return fs_ptr;
}

void lowfat_fs_set_instance_addresses(lowfat_fs* fs_ptr) {
    // 40 bytes used for common fs values
    const uint32_t fileinfo_stride = sizeof(lowfat_fs_fileprops_t) + fs_ptr->_header->_filename_length;
    const uint32_t fileinfo_size = fileinfo_stride * fs_ptr->_header->_cluster_count;
    fs_ptr->_filenames = fs_ptr->_data + sizeof(lowfat_fs_header);
    fs_ptr->_fileprops = fs_ptr->_data + sizeof(lowfat_fs_header) + fs_ptr->_header->_filename_length;
    fs_ptr->_filename_table = (structures_int_pair_t*)(fs_ptr->_data + sizeof(lowfat_fs_header) + fileinfo_size);
...
void lowfat_fs_reset_instance(lowfat_fs* fs_ptr) {
    ...
    fs_ptr->_header->_filename_table_busy_tail = LOWFAT_FS_NONE;
    fs_ptr->_header->_data_table_busy_tail = LOWFAT_FS_NONE;
    const uint32_t fileinfo_stride = sizeof(lowfat_fs_fileprops_t) + fs_ptr->_header->_filename_length;
    ...
    fs_ptr->_filename_table[fs_ptr->_header->_cluster_count - 1].PAIR_NEXT = LOWFAT_FS_NONE;
    ...
    fs_ptr->_filename_table[fs_ptr->_header->_filename_table_busy_tail].PAIR_NEXT = LOWFAT_FS_NONE;
    fs_ptr->_filename_table[fs_ptr->_header->_filename_table_free_head].PAIR_PREV = LOWFAT_FS_NONE;
    ...
    fs_ptr->_data_table[fs_ptr->_header->_cluster_count - 1].PAIR_NEXT = LOWFAT_FS_NONE;
    ...
    fs_ptr->_data_table[fs_ptr->_header->_data_table_busy_tail].PAIR_NEXT = LOWFAT_FS_NONE;
    fs_ptr->_data_table[fs_ptr->_header->_data_table_free_head].PAIR_PREV = LOWFAT_FS_NONE;
}
...
int32_t lowfat_fs_open_file(lowfat_fs* fs_ptr, const char* filename, char mode) {
    ...
    if (mode == 'r') {
        ...
        if (fs_ptr->_header->_data_table_free_head == LOWFAT_FS_NONE) {
        ...
        lowfat_fs_dl_acquire_next_free(fs_ptr->_filename_table, &fs_ptr->_header->_filename_table_busy_tail, &fs_ptr->_header->_filename_table_free_head);
        fd = fs_ptr->_header->_filename_table_busy_tail;
        // put busy node to the head of list
        lowfat_fs_dl_acquire_next_free(fs_ptr->_data_table, &fs_ptr->_header->_data_table_busy_tail, &fs_ptr->_header->_data_table_free_head);
    ...
    if (fd >= 0) {
        ...
        while (total_write_size > 0) {
            ...
            if (mem_can_write == 0) {
                ...
                LOWFAT_FS_ASSERT(fs_ptr->_header->_data_table_free_head != LOWFAT_FS_NONE);
                if (fs_ptr->_header->_data_table_free_head == LOWFAT_FS_NONE) {
                    return LOWFAT_FS_ERROR_SPACE_ENDED;
                }
                lowfat_fs_dl_acquire_next_free(fs_ptr->_data_table, &fs_ptr->_header->_data_table_busy_tail, &fs_ptr->_header->_data_table_free_head);
...
uint32_t lowfat_fs_remove_file(lowfat_fs* fs_ptr, int32_t fd) {
    LOWFAT_FS_ASSERT(fd >= 0 && fd < (int32_t)fs_ptr->_header->_cluster_count);
    if (fd >= 0 && fd < (int32_t)fs_ptr->_header->_cluster_count) {
        ...
        lowfat_fs_dl_free_busy_range(fs_ptr->_data_table, first_cluster, last_cluster, &fs_ptr->_header->_data_table_busy_tail, &fs_ptr->_header->_data_table_free_head);
        // 
...
uint32_t lowfat_fs_walk_over_all_files(const lowfat_fs* const fs_ptr, void* arg, void(*procedure)(int32_t fd, void* arg)) {
    if (fs_ptr->_header->_file_count) {
        int32_t cur_fd = fs_ptr->_header->_filename_table_busy_tail;
        while (cur_fd != LOWFAT_FS_NONE && (uint32_t)cur_fd < fs_ptr->_header->_cluster_count && (uint32_t)cur_fd > fs_ptr->_info._last_system_cluster) {
